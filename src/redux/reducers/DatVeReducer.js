...
export const DatVeReducer = (state = stateDefault, action) => {
    let { type, ghe, soGhe } = action
    switch (type) {
        case DAT_GHE: {
            let danhSachGheDangDat = [...state.danhSachGheDangDat];
            let gheDangDat = danhSachGheDangDat.find(item => item.soGhe == ghe.soGhe)
            if (gheDangDat) {
                danhSachGheDangDat = danhSachGheDangDat.filter(item => item.soGhe !== gheDangDat.soGhe)
            }
            else {
        ...
        case HUY_GHE: {
            let danhSachGheDangDat = [...state.danhSachGheDangDat];
            let index = danhSachGheDangDat.findIndex(item => item.soGhe == soGhe)
            if (index !== -1) {
        ...
        case THANH_TOAN: {
            ...
            danhSachGheDangDat.forEach((gheDangDat) => {
                    danhSachGheData.forEach((hangGheDaDat) => {
                        hangGheDaDat.danhSachGhe.forEach((gheDaDat) => {
                            if(gheDangDat.soGhe == gheDaDat.soGhe){
                                gheDaDat.daDat = true
                            }
                        })
                    } )
            ...
            danhSachGheDangDat.splice(0, danhSachGheDangDat.length);

            return { ...state, danhSachGheData,danhSachGheDangDat }
        }
